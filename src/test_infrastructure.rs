		...
		for level1 in 0..20 {
			...
			for level2 in 0..10 {
				...
				for file_num in 0..5 {
					...
					let content = format!(
						"Content for level1={} level2={} file={}",
						level1, level2, file_num
					);
...
mod tests {
	...
	fn test_small_structure_creation() {
		let structure =
			TestDirectoryStructure::create_small().expect("Failed to create small structure");
	...
	fn test_medium_structure_creation() {
		let structure =
			TestDirectoryStructure::create_medium().expect("Failed to create medium structure");
	...
	fn test_edge_cases_structure() {
		let structure =
			TestDirectoryStructure::create_edge_cases().expect("Failed to create edge cases");

		// Verify edge case files exist
		assert!(structure.root().join("empty_dir").is_dir());
		assert!(structure
			.root()
			.join("dir with spaces & special chars")
			.is_dir());
		assert!(structure
			.root()
			.join("üñíçødé_dir")
